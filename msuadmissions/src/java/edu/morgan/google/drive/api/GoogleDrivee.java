...
public class GoogleDrivee {
    ...
    public GoogleDrivee(String codeValidation) {
    ...
    public File getFolderByStudentInfo(String lastName, String firstName, String studentID) {
        try {
            ArrayList<File> files = (ArrayList) retrieveAllFiles("title contains '" + lastName.toLowerCase().trim() + "' and mimeType = 'application/vnd.google-apps.folder'");
            for (File filed : files) {
                String title = filed.getTitle().toLowerCase().replaceAll("_", " ");
                if (!firstName.equals("") && title.contains(firstName.toLowerCase().trim()) && title.contains(studentID.trim())) {
                    return filed;
                }
            }
            return null;
    ...
    public File getFolderByStudentInfo(String lastName, String firstName) {
        try {
            ArrayList<File> files = (ArrayList) retrieveAllFiles("title contains '" + lastName.toLowerCase().trim() + "' and mimeType = 'application/vnd.google-apps.folder'");
            for (File filed : files) {
                String title = filed.getTitle().toLowerCase().replaceAll("_", " ");
                if (!firstName.equals("") && title.contains(firstName.toLowerCase().trim())) {
                    return filed;
                }
            }
            return null;
